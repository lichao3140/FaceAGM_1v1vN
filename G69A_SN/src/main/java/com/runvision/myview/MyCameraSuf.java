...
public class MyCameraSuf extends SurfaceView implements SurfaceHolder.Callback, Camera.PreviewCallback {
    ...
    public static byte[] getCameraData() {
        if (cameraData == null) {
            ...
            return CameraHelp.rotateCamera(cameraData, Const.PRE_WIDTH, Const.PRE_HEIGTH, 270);
        }
    }
    ...
    public void openCamera() {
        System.out.println("openCamera" + flag);
        if (flag != false) {
            return;
        }
        try {
    ...
    private void initCamera() {
        if (mCamera != null) {
            try {
                ...
                if (camerType == 0) {
                    FrameLayout.LayoutParams lp = new FrameLayout.LayoutParams(mScreenWidth, mScreenHeight);
                    this.setLayoutParams(lp);
                } else {
                    FrameLayout.LayoutParams lp = new FrameLayout.LayoutParams(384, 512);
                    this.setLayoutParams(lp);
                }
                ...
                if (camerType == 0) {
                    ...
                    Const.Panel_width = mScreenWidth;
                    mProportionH = (float) mScreenWidth / (float) Const.PRE_HEIGTH;
                    mProportionW = (float) mScreenHeight / (float) Const.PRE_WIDTH;
                } else {
