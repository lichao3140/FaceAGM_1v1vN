...
//        String sql = "select * from tRecord order by id desc limit " + pageSize + " offset " + (pageIndex - 1) * pageSize;
//        mList = MyApplication.faceProvider.queryRecord(sql);
//        int num = MyApplication.faceProvider.quaryRecordTableRowCount("select count(*) from tRecord");
//        if (num % pageSize == 0) {
//            row = (num / pageSize);
//        } else {
//            row = (num / pageSize) + 1;
...
//    }
//
//    private View showOneVSMoreDialog(User user) {
//        //加载视图
...
//
//
//        Bitmap snapImage = FileUtils.loadTempBitmap(user.getRecord().getSnapImageID(), Const.SNAP_DIR);
//        onevsmore_face.setImageBitmap(snapImage);
//        if(user.getRecord().getComperType().equals("人证")){
//            onevsmore_userType.setText(user.getRecord().getComperType());
//
...
//            Bitmap tempImage = FileUtils.loadTempBitmap(user.getTemplateImageID(),Const.TEMP_DIR);
